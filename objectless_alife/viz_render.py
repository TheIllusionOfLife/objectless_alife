...
def _aggregate_metric_data(
    ...
    for label, metrics_path in phase_data:
        final_table = load_final_step_metrics(metrics_path)
        try:
            col = final_table.column(metric_name).combine_chunks()
        except KeyError:
            all_data.append([])
            labels.append(label)
            continue

        values = col.cast(pa.float64())
        # Filter finite values (excludes NaN, Inf, and nulls)
        mask = pc.is_finite(values).fill_null(False)
        valid_values = values.filter(mask)

        all_data.append(valid_values.to_pylist())
        labels.append(label)
