...
achieve nonzero median excess MI (MI calibrated by a permutation-based shuffle
null that controls for pair-count bias) while
density-only and control agents remain at zero
(Cliff's $\delta = 0.37$; bootstrap 95\% CI for median difference excludes zero),
...
pair-count bias ($\mathrm{MI}_{\mathrm{excess}} \approx 0$).
Moran's $I$ independently confirms spatial clustering accompanies elevated MI.
...
computing $\hat{I}_{\mathrm{MM}}$ for each shuffle.  The mean across shuffles
is the shuffle-null MI.  The \emph{excess MI},
$\mathrm{MI}_{\mathrm{excess}} = \hat{I}_{\mathrm{MM}} -
\overline{I}_{\mathrm{shuffle}}$, isolates genuine spatial coordination from
estimation artifacts.  In particular, the random walk's high raw MI
($\approx 0.36$~bits) is entirely attributable to pair-count bias:
$\mathrm{MI}_{\mathrm{excess}} \approx 0$.

\paragraph{Moran's I.}
Spatial autocorrelation is quantified by Moran's $I$ across occupied cells
using torus 4-neighborhood weights: both cells must be occupied for a
weight of~1.  Values range from $-1$ (dispersed) through $0$ (random) to $+1$
(clustered).  Moran's~$I$ provides an independent confirmation that elevated
MI reflects genuine spatial clustering, not a statistical artifact.
...
$\delta$~(equivalent to rank-biserial correlation for Mann-Whitney $U$):
...
measures, we report bootstrap 95\% confidence intervals for the median
difference (10{,}000 resamples, percentile method) as the primary effect-size
...
\begin{table}[htbp]
\centering
\caption{Neighbor mutual information and spatial clustering by condition.
MI values use the Miller-Madow bias-corrected estimator;
MI\textsubscript{excess} = MI\textsubscript{observed} $-$ MI\textsubscript{shuffle null}
controls for pair-count bias (5{,}000 rules per condition, $N=200$ shuffles).
The random walk's high raw MI is entirely attributable to pair-count bias
(MI\textsubscript{excess} $\approx 0$).}
\label{tab:mi_summary}
\begin{tabular}{lccccc}
\toprule
Condition & Table Size & Median MI & Median MI\textsubscript{excess} & Median Moran's $I$ & Survival \\
\midrule
Random Walk & 1 (unused) & 0.910 & 0.050  & $-0.030$ & 100.0\% \\
Control     & 100        & 0.000 & 0.000  & 0.124    & 44.5\% \\
Phase 1     & 20         & 0.019 & 0.000  & $-0.011$ & 71.4\% \\
Phase 2     & 100        & 0.298 & 0.096  & $-0.020$ & 74.7\% \\
\bottomrule
\end{tabular}
\end{table}

The control condition, despite having 100-entry tables (equal to Phase~2),
produces zero median excess MI---demonstrating that table size alone is insufficient.
Phase~2 is the only rule-based condition with nonzero median excess MI,
...
null hypothesis of no spatial state structure. The excess MI
($\mathrm{MI}_{\mathrm{excess}} = \mathrm{MI}_{\mathrm{observed}} -
\overline{\mathrm{MI}}_{\mathrm{shuffle}}$) isolates genuine coordination
from pair-count artifacts.

For the random walk, $\mathrm{MI}_{\mathrm{excess}} \approx 0$
...
contrast, Phase~2 retains substantial excess MI, demonstrating genuine
coordination that persists after bias calibration. Moran's $I$ independently
corroborates this: Phase~2 shows positive spatial autocorrelation while the
...
\begin{table}[htbp]
\centering
\caption{Pairwise statistical tests for neighbor MI.
Effect sizes are Cliff's $\delta$ (= rank-biserial $r$).
Bootstrap 95\% CIs for the median difference (10{,}000 resamples) are the
primary effect-size statistic; $p$-values (Holm-Bonferroni corrected) are
secondary.}
\label{tab:stats}
\begin{tabular}{llcccc}
\toprule
Comparison & Direction & Cliff's $\delta$ & Median diff 95\% CI & $p$-value \\
\midrule
P1 vs.\ P2           & P2 $>$ P1      & 0.339 & [0.260, 0.295] & $< 10^{-199}$ \\
P1 vs.\ Control      & P1 $>$ Ctrl    & 0.270 & [0.012, 0.026] & $< 10^{-158}$ \\
Control vs.\ P2      & P2 $>$ Ctrl    & 0.482 & [0.281, 0.311] & $\approx 0$ \\
\bottomrule
\end{tabular}
\end{table}
...
only rule-based condition achieving nonzero median excess MI, while
both Phase~1 and Control fall to zero---making the qualitative separation
between observation conditions unambiguous.
Moran's $I$ independently confirms that elevated MI corresponds to genuine
spatial clustering, not a statistical artifact.
