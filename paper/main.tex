...
with viability-only filtering, comparing four observation conditions:
...
(Cliff's $\delta = 0.36$; median-difference bootstrap 95\% CI $[0.078, 0.111]$; Table~\ref{tab:stats}),
...
We explore the unexplored quadrant of \emph{no objective $\times$ viability-only
selection}, where the only filtering criterion is physical
consistency---removing rules that produce trivially broken simulations (all
agents halt or converge to a single state). This viability-only filtering is
analogous to the laws of physics: it constrains what is possible without
specifying what is desirable. Importantly, this negative selection does
constitute a minimal selection pressure (surviving rules must avoid
degenerate dynamics), but no behavioral or performance criterion is employed.
...
\caption{Grid snapshots of hand-picked highest-MI surviving rules from each
condition (final-step grid; Miller-Madow estimator).
...
Parameter & Value \\
\midrule
Grid topology       & $20 \times 20$ torus (von~Neumann neighborhood) \\
...
is the shuffle-null MI.  The \emph{excess MI} is then:
\begin{equation}
  \mathrm{MI}_{\mathrm{excess}} = \max\!\bigl(\hat{I}_{\mathrm{MM}} -
  \overline{I}_{\mathrm{shuffle}},\; 0\bigr)
  \label{eq:mi_excess}
\end{equation}
...
\begin{table}[htbp]
\centering
\caption{Pairwise statistical tests for $\mathrm{MI}_{\mathrm{excess}}$
(Eq.~\ref{eq:mi_excess}).
...
\midrule
P1 vs.\ P2      & P2 $>$ P1   & 5000 & 5000 & 0.355 & [0.078, 0.111] & $< 10^{-252}$ \\
Ctrl vs.\ P1    & P1 $>$ Ctrl & 5000 & 5000 & 0.124 & [$< 0.001$, $< 0.001$] & $< 10^{-50}$ \\
Ctrl vs.\ P2    & P2 $>$ Ctrl & 5000 & 5000 & 0.423 & [0.078, 0.111] & $\approx 0$ \\
\bottomrule
\end{tabular}
\end{table}
...
We note that Moran's $I$ treats categorical states as numeric, which is
inappropriate for nominal data; the same-state adjacency fraction
(Section~3.4) provides the correct categorical spatial statistic and
confirms local coordination in Phase~2 (see Appendix~\ref{app:multi_seed}).
...
$r = 0.45$ for Control. The modest correlation for Phase~2 indicates that
while some relationship exists (surviving rules tend to have slightly higher MI),
the MI advantage of Phase~2 over Phase~1 is not an artifact of differential
filter selection: both conditions have nearly identical filter-MI correlation
($r \approx 0.14$), yet Phase~2 achieves substantially higher MI_excess.
...
\begin{itemize}
  ...
  \item \textbf{Single simulation per rule (main experiment)}: Each rule table
        ...
        Appendix~\ref{app:multi_seed} provides multi-seed robustness analysis
        for the top 50 Phase~2 rules, confirming that high MI is a property
        of the rule table, not a seed-specific accident.
\end{itemize}
...
$\mathrm{MI}_{\mathrm{excess}}$ (Eq.~\ref{eq:mi_excess}) values are
...
%% =========================================================================
%% APPENDIX B: MULTI-SEED ROBUSTNESS
%% =========================================================================
