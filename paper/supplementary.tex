...
Rules with median $\mathrm{MI}_{\mathrm{excess}} > 0$
  & 41/50 (82\%) & \textit{TBD} & \textit{TBD} \\
Mean $P(\mathrm{MI}_{\mathrm{excess}} > 0)$ across seeds
  & 0.733 & \textit{TBD} & \textit{TBD} \\
Overall survival rate
  & 76.2\% & \textit{TBD} & \textit{TBD} \\
\bottomrule
\end{tabular}
\end{table*}
...
73.3\% of seeds per rule produce positive excess MI.  Phase~1 and Control
results (to be populated after running
\texttt{scripts/multi\_seed\_p1\_control.py}) are expected to confirm that
their low or zero MI levels are equally stable, completing the evidence that
MI differences across conditions reflect genuine rule properties rather than
seed-specific accidents.
...
families at every time step.  Table~\ref{tab:metric_profiles} reports
final-step values for surviving rules across all four conditions.  Values
are to be populated after running \texttt{scripts/metric\_comparison.py}.

\begin{table*}[htbp]
\centering
\caption{Cross-condition metric profiles (surviving rules, final-step
values).  Median [Q1, Q3] reported.  Mann-Whitney $U$ $p$-values
(Holm-Bonferroni corrected) test P1 vs.\ P2 and Control vs.\ P2.}
...
Action entropy (mean)
  & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
Action entropy (variance)
  & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
Cluster count
  & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
Quasi-periodicity peaks
  & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
Phase transition $\max \Delta$
  & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
State entropy
  & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\bottomrule
...
emergent role differentiation.  Phase~2's richer observation channel,
which includes dominant neighbor state, is expected to support greater
role differentiation than Phase~1 or Control, where agents cannot
distinguish neighbor identity.
...
Quasi-periodicity peaks indicate recurrent oscillatory patterns in
state-entropy time series, while $\max \Delta$ records the largest
single-step entropy change---a proxy for abrupt reorganization events.
Differences across conditions in these temporal metrics complement the
spatial coordination story told by MI.
...
Values are to be populated after running
\texttt{scripts/cascaded\_filter\_analysis.py}.
...
(Control $<$ P1 $<$ P2) is expected to persist.}
\label{tab:cascade_survival}
\begin{tabular}{lccc}
\toprule
Condition & Weak Only & Weak+Medium & $\Delta$ \\
\midrule
Phase~1 & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
Phase~2 & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
Control & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\bottomrule
\end{tabular}
\end{table}

The cascaded filter experiment tests whether the central finding---that
observation richness drives emergent coordination---holds under stricter
viability criteria.  If the Phase~2 MI advantage persists among
medium-filter survivors, it strengthens the claim that coordination is a
genuine property of the rule--observation interaction, not an artifact of
lax filtering.
