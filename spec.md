...
統計的検定にはMann-Whitney U検定（Holm-Bonferroni補正）を使用する。MIの推定にはMiller-Madowバイアス補正を適用する。さらに、シャッフルヌル（占有位置固定・状態順列置換、N=200）によりペア数バイアスを校正し、MI_excess = MI_observed - MI_shuffle_null を主指標とする。効果量はCliff's δとブートストラップ95%信頼区間で報告する。
...
| 指標 | 計算方法 |
|------|---------|
| 近傍相互情報量 | 隣接セル間の内部状態の相互情報量 I(S_i; S_j)。全隣接ペアの平均。Miller-Madowバイアス補正を適用。 |
| シャッフルヌルMI | 占有位置を固定し、状態をランダムに再割り当てした場合のMIの平均値（N=200回のシャッフル）。ペア数バイアスの校正に使用。MI_excess = MI_observed - MI_shuffle_null がバイアス補正済みの指標となる。 |

### 7.6 統計的検定

| 手法 | 説明 |
|------|------|
| Mann-Whitney U検定 | 二条件間のメトリック比較。Holm-Bonferroni多重比較補正を適用。 |
| 効果量 | Cliff's δ（= rank-biserial correlation r）。Mann-Whitney U統計量から算出: r = 1 - 2U/(n₁n₂)。 |
| ブートストラップCI | 中央値差の95%信頼区間。10,000回リサンプリングによるパーセンタイル法。 |
| 生存率検定 | カイ二乗検定（2×2分割表）。 |
